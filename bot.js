...
bot.command("addcredits", async (ctx) => {
  ...
  // Notify the user
  try {
    await bot.telegram.sendMessage(targetId, `🎁 *${amount} credits* have been added to your account!\n\nNew balance: *${user.credits + amount} credits*`, { parse_mode: "Markdown" });
  } catch(e) {}

  ctx.reply(`✅ Added *${amount}* credits to user *${user.username || targetId}*\nNew balance: *${user.credits + amount}*`, { parse_mode: "Markdown" });
});

// /setcredits [telegram_id] [amount]
bot.command("setcredits", async (ctx) => {
  ...
  ctx.reply(`✅ Set credits to *${amount}* for user *${user.username || targetId}*`, { parse_mode: "Markdown" });
});
